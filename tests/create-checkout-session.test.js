...
test("checkout records that artwork was selected without receiving the file bytes",async()=>{const calls={};const handler=createCheckoutSessionHandler({env:baseEnv,stripeFactory:createStripeFactory(calls)});const res=await invoke(handler,{lead:{serviceType:"Business cards",checkoutOptionId:"cards-250"},artworkExpected:true});assert.equal(res.statusCode,200);assert.equal(calls.payload.metadata.artworkState,"selected_pending_payment");assert.equal(calls.payload.metadata.artworkExpected,"yes")});
