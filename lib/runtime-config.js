...
const DEFAULT_BUSINESS_CARDS_CENTS = 5900;
...
const BUNDLE_TIER_DEFAULTS = Object.freeze([
  ...
  {
    id: "bundle-3-250",
    label: "3 tents + 250 cards",
    quantityLabel: "3 tents + 250 cards",
    amountCents: 275000,
...
function createServiceOptions(env, options) {
  return options.map(({ envKey, legacyEnvKey, ...defaultOption }) => {
    const envValue = env[envKey] || (legacyEnvKey ? env[legacyEnvKey] : undefined);
    return createTierOption(defaultOption, envValue);
  });
}
...
function createStripeCatalog(env = process.env) {
  ...
  const businessCardOptions = createServiceOptions(env, [
    ...
    { ...BUSINESS_CARD_TIER_DEFAULTS[2], envKey: "STRIPE_BUSINESS_CARDS_250_CENTS", legacyEnvKey: "STRIPE_BUSINESS_CARDS_200_CENTS" },
    {
  ...
  const bundleOptions = createServiceOptions(env, [
    ...
    { ...BUNDLE_TIER_DEFAULTS[1], envKey: "STRIPE_BUNDLE_3_TENTS_250_CARDS_CENTS", legacyEnvKey: "STRIPE_BUNDLE_3_TENTS_200_CARDS_CENTS" },
...
function createPublicRuntimeConfig(env = process.env) {
  ...
  return {
    ...
    artworkUploadConfigured: Boolean(env.GMAIL_USER && env.GMAIL_APP_PASSWORD),
    maxArtworkBytes: 2500000,
